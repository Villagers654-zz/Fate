...
class Minecraft(commands.Cog):
	...
	async def ritter_api(self, ctx, player_name=None):
		if not player_name:
			...
			e.set_author(name='2B2T Queue', icon_url='https://cdn.discordapp.com/avatars/79305800157233152/a_1d26f615a6271e4e15216b0a01c5e03e.gif?size=1024')
		...
		e = discord.Embed(color=colors.green())
		e.set_author(name=f"{player_name} Stats", icon_url="attachment://" + os.path.basename(path))
		e.description = f"Kills: {dat['kills']}" \
		                f"\nDeaths: {dat['deaths']}" \
		                f"\nJoins: {dat['joins']}"
		await ctx.send(embed=e, file=discord.File(path, filename=os.path.basename(path)))
