...
class Utility(commands.Cog):
    ...
    async def info(self, ctx, *, target=None):
        ...
        if "discord.gg" in target:
            ...
            e.set_thumbnail(
                url=guild.splash_url if guild.splash_url else guild.icon_url
            )
        ...
        if ctx.message.mentions:  # user mentions
            ...
            main = (
                f'{f"**• Nickname** [`{user.nick}`]" if user.nick else ""}\n'
            ...
            for role_list in roles:
                ...
                e.add_field(
                    name=f"◈ Roles ◈ ({len(user.roles)})" if index is 0 else "~",
                    value=role_list,
                    inline=False,
                )
            ...
            perms = ", ".join(
                perm for perm, value in permissions if value and perm in notable
            )
            ...
            e.add_field(
                name="◈ Created ◈",
                value=datetime.date(user.created_at).strftime("%m/%d/%Y"),
                inline=False,
            )
            return await ctx.send(embed=e)
        if ctx.message.channel_mentions:
            ...
            e.add_field(
                name="◈ Created ◈",
                value=datetime.date(channel.created_at).strftime("%m/%d/%Y"),
                inline=True,
            )
            return await ctx.send(embed=e)
        ...
        perms = ", ".join(
            perm for perm, value in role.permissions if value and perm in notable
        )
        perms = "administrator" if role.permissions.administrator else perms
        e.add_field(
            name="◈ Perms ◈", value=f"```{perms if perms else 'None'}```", inline=False
        )
        e.add_field(
            name="◈ Created ◈",
            value=datetime.date(role.created_at).strftime("%m/%d/%Y"),
            inline=False,
        )
    ...
    async def avatar(self, ctx, *, user=None):
        ...
        if not user.avatar_url:
            return await ctx.send(
                f"{utils.cleanup_msg(ctx.message, user.display_name)} doesn't have an avatar"
            )
        e = discord.Embed(color=0x80B0FF)
    ...
    async def poll(self, ctx, *, arg):
        ...
        e.set_author(
            name="| {} |".format(ctx.author.name), icon_url=ctx.author.avatar_url
        )
