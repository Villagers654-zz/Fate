...
class Utility(commands.Cog):
	...
	async def info(self, ctx, users: g[User], roles: g[Role], channels: g[TextChannel]):
		...
		if users:
			...
			e.add_field(name="◈ Invite Information", value=self.bot.utils.format_dict(core), inline=False)
			if inviters:
					...
					try:
						...
						for key, value in invite_info.items():
							try:
								...
								elif value != self.misc_logs["invites"][invite.code][key] and value is not None:
									self.misc_logs["invites"][invite.code][key] = value
							except KeyError:
								return
						if msg.author.id not in self.misc_logs["invites"][invite.code]["inviters"]:
							self.misc_logs["invites"][invite.code]["inviters"].append(msg.author.id)
