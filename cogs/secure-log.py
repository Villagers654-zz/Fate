...
class SecureLog(commands.Cog):
    ...
    async def start_queue(self, guild_id: str):
        ...
        if guild_id not in self.recent_logs:
            if self.config[guild_id]["type"] == "single":
                ...
                self.recent_logs[guild_id] = {Type: [] for Type in self.channel_types}
        ...
        while True:
            ...
            for embed, channelType, logged_at in self.queue[guild_id][-175:]:
                ...
                while not category:
                    try:
                        category = await self.bot.fetch_channel(
                ...
                try:
                    if isinstance(category, discord.TextChannel):  # single channel log
                        try:
                            ...
                            e.description = (
                                f"```{json.dumps(embed.to_dict(), indent=2)}```"
                            )
    ...
    async def search_audit(self, guild, *actions):
        ...
        if guild.me.guild_permissions.view_audit_log:
            ...
            await guild.owner.send(
                f"I'm missing audit log permissions for secure-log in {guild}\n"
    ...
    async def on_ready(self):
        for guild_id in self.config.keys():
            self.bot.tasks.start(
                self.start_queue, guild_id, task_id=f"queue-{guild_id}"
            )
    ...
    async def on_raw_bulk_message_delete(self, payload):
        ...
        if guild_id in self.config:
            ...
            if dat["extra"] and dat["icon_url"]:
                e.set_author(
                    name=f"~==🍸{dat['extra']['count']} Msgs Purged🍸==~",
                ...
                for obj, permissions in list(before.overwrites.items()):
                    ...
                    if list(permissions) != after_values:
                        ...
                        e.add_field(
                            ...
                            value="\n".join(
                                [
                                    f"{perm} - {after_values[iter][1]}"
                                    for iter, (perm, value) in enumerate(
                                        list(permissions)
                                    )
                                    if (value != after_values[iter][1])
                                ]
                            ),
                            inline=False,
                        )
            ...
            if before.color != after.color:
                ...
                def font(size):
                    return ImageFont.truetype(
                        "./utils/fonts/Modern_Sans_Light.otf", size
                    )
