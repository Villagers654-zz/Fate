...
class Emojis(commands.Cog):
    ...
    async def _add_emoji(
        ...
        for emoji in custom:
            ...
            await self.upload_emoji(
                ctx, name=name, img=img, reason=str(ctx.author), msg=msg
            )
        ...
        for emoji_id in ids:
            ...
            img = requests.get(emoji.url).content
            await self.upload_emoji(
                ctx, name=str(emoji_id), img=img, reason=str(ctx.author), msg=msg
            )
        ...
        mappings = {
            requests.get(arg).content: check(iter)
            if "." not in check(iter)
            else "new_emoji"
    ...
    async def add_emoji(self, ctx, *, args="new_emoji"):
        ...
        if "https://" in args[0]:
            image_urls = args[0]
            args.pop(0)
            args = ["new_emoji"] if not args else args
        ...
        if not isinstance(image_urls, list):
            ...
            name = self.cleanup_text(args)
            img = requests.get(image_urls).content
            ...
            for filename, url in image_urls:
                ...
                name = self.cleanup_text(args if args else filename)
                img = requests.get(url).content
                await self.upload_emoji(ctx, name, img, str(ctx.author), roles, msg)
                await asyncio.sleep(1)

    @commands.command(
        name="stealemoji", aliases=["stealemote", "fromemote", "fromemoji"]
    )
    @commands.has_permissions(manage_emojis=True)
    @commands.bot_has_permissions(manage_emojis=True)
    @commands.cooldown(1, 5, commands.BucketType.guild)
