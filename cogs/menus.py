...
class Menus(commands.Cog):
    ...
    async def _help(self, ctx):
        if not ctx.invoked_subcommand:
            ...
            e.add_field(
                name="◈ Responses ◈",
                value="`@Fate` `hello` `ree` `kys` `gm` `gn`",
                inline=False,
            )
            e.add_field(
                name="◈ Music ◈",
    ...
    @commands.command(
        name="stats", description="Provides information relevant to the bots stats"
    )
    ...
    async def stats(self, ctx):
        ...
        path = (
            os.getcwd()
            + "/data/images/banners/"
            + random.choice(os.listdir(os.getcwd() + "/data/images/banners/"))
        )
