...
class Menus(commands.Cog):
	...
	async def _help(self, ctx, command=None):
		...
		e.add_field(name="◈ Core ◈", value="`leaderboard` `gleaderboard` `ggleaderboard` `mleaderboard` `gmleaderboard` `vcleaderboard` `gvcleaderboard` `editconfig` `changelog` `partners` `discords` `servers` `restrict` `config` `prefix` `realms` `links` `ping` `info` `say`", inline=False)
