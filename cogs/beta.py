...
class UtilityBeta(commands.Cog):
            ...
            if isinstance(arg, discord.Guild):
                ...
                if guild_id not in self.guild_logs:
                    self.guild_logs[guild_id] = {
                        ...
                        "bots": {
                            str(bot.id): None
                            for bot in [m for m in arg.members if m.bot]
                        },
                    }
    ...
    async def xinfo(
        self, ctx, users: g[User], roles: g[Role], channels: g[TextChannel]
    ):
        if users:
            user = users[0]
            if ctx.guild:
                    ...
                    if user_id not in logs or not logs[user_id]:
                        ...
                        if ctx.guild.me.guild_permissions.view_audit_log:
                            async for entry in ctx.guild.audit_logs(
                                limit=50, action=discord.AuditLogAction.bot_add
                            ):
                                if entry.target and entry.target.id == user.id:
                                    self.guild_logs[guild_id]["bots"][
                                        user_id
                                    ] = entry.user.id
                                    break
                    ...
                    try:
                        ...
                        for key, value in invite_info.items():
                            if key not in self.misc_logs[invite.code]:
                                ...
                                value != self.misc_logs[invite.code][key]
