...
class Fun(commands.Cog):
    ...
    async def battle(self, ctx, user1: discord.User, user2: discord.User = None):
        ...
        attacks = {
            ...
            "🎱 | !user summoned the Magic 8 Ball to erase !target `-420HP`": 420,
            "🗡️ | !user injected !target with uncle's secret recipe `-27HP`": 27,
            "📸 | !user caught !target in 4k `-50HP`": 50,
            "👽 | !target was abducted by Jeff `-4000HP`": 4000,
            "👾 | !user fed !target to the Martians above her kitchen `-49HP`": 49,
            "💳 | !user swiped both of !target's credit cards painfully `-15HP`": 15,
            "🦈 | !user morphed into sharky and swallowed !target calmly `-75HP`": 75,
            "🐦 | !user ratioed !target on twitter `-10HP`": 10,
            "🍆 | !user scared !target away with his extra large benis `-21HP`": 21,
            ...
            "💩 | Villagers654 ruined the vibe causing !target to breakdown `-21HP`": 21,
            "🐄 | Luck's instructions were unclear leaving !target without food and shelter `-33HP`": 33,
            "📉 | Legit told !target to buy all the dips but !target pressed the wrong button `-25HP`": 25,
