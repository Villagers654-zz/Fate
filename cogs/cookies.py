...
class Cookies(commands.Cog):
	...
	async def save(self):
		data = {
			"cookies": self.cookies, "sent": self.sent, "received": self.received, "eaten": self.eaten, "cd": self.cd
		}
	...
	async def _cookie(self, ctx, user: discord.Member=None):
		...
		if user:
			...
			e.set_author(name=f"| 📤 {self.sent[author_id]} | 📥 {self.received[author_id]} | 🍪 {self.cookies[author_id]}", icon_url=ctx.author.avatar_url)
