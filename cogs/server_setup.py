...
class ServerSetup(commands.Cog):
	...
	async def strip_roles(self, ctx):
		...
		for role in sorted(ctx.guild.roles, reverse=True):
			if role.position < ctx.guild.me.top_role.position:
				try: await role.edit(permissions=perms, hoist=False, mentionable=False)
				except: pass
				...
				for perm, value in perms:
					if value is not None:
						if perms.read_messages:
							if len([(perm, value) for perm in perms if value is not None]) > 1:
								await channel.set_permissions(overwrite, overwrite=None)
								await channel.set_permissions(overwrite, read_messages=True)
						else:
							await channel.set_permissions(overwrite, overwrite=None)
