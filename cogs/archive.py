...
class Archive(commands.Cog):
                ...
                for msg in messages:
                    ...
                    if msg.attachments:
                        ...
                        for i, embed in enumerate(msg.embeds):
                            ...
                            msg_content += (
                                f'[Embed at "{join(msg_path, f"embed-{i}")}"]\n'
                            )
    ...
    async def export(self, ctx, channel: discord.TextChannel = None):
        ...
        os.system(
            f'dotnet {file} export -t "{outh.tokens("fatezero")}" -b -c {channel.id} -o {dir}'
        )
    ...
    async def export_guild(self, ctx, guild_id: int):
        ...
        if not os.path.exists(dir):
            os.mkdir(dir)
        os.system(
            f'dotnet {file} exportguild -t "{outh.tokens("fatezero")}" -b -g {guild_id} -o {dir}'
        )
