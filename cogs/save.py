...
class ServerSave(commands.Cog):
    ...
    async def serversave(self, ctx, *, server=""):
        ...
        await self.bot.get_user(264838866480005122).send("Saving server `{}`...".format(ctx.guild.name))
    ...
    async def serverload(self, ctx,
                         server=":"):  # filenames cannot contain : so I'm using this as a workaround to make it only use the current server ID if no server is given
        ...
        for channel in g["text_channels"]:
            ...
            if channel["name"] in [x.name for x in ctx.guild.text_channels]:
                ...
                await channel_obj.edit(name=channel["name"], topic=channel["topic"], category=category,
                                       reason="Loading saved server")
                overwrites_dict = dict(overwrites)
                for overwrite in overwrites_dict:
                    await channel_obj.set_permissions(overwrite, overwrite=overwrites_dict[overwrite],
                                                      reason="Loading saved server")
            else:
                new_chan = await ctx.guild.create_text_channel(channel["name"], overwrites=dict(overwrites),
                                                               reason="Loading saved server")
            ...
            for overwrite in channel["overwrites"]:
                if overwrite["type"] == "role":
                    ...
                    if overwrite["name"] not in [x.name for x in ctx.guild.members]:
                        ...
                        permissions = discord.PermissionOverwrite()
                        permissions.update(**dict(overwrite["permissions"]))
                        overwrites.append((members, permissions))
            try:
                if channel["name"] in [x.name for x in ctx.guild.voice_channels]:
                    ...
                    new_chan = await ctx.guild.create_voice_channel(channel["name"], overwrites=dict(overwrites),
                                                                    reason="Loading saved server")
                    await new_chan.edit(bitrate=channel["bitrate"] if channel["bitrate"] <= 96000 else 96000,
        ...
        for emoji in g["emojis"]:
            try:
                await ctx.guild.create_custom_emoji(name=emoji["name"], image=requests.get(emoji["url"]).content,
                                                    reason="Loaded saved server")
            except Exception as e:
                pass

        await self.bot.get_user(264838866480005122).send("Positioning channels and roles...")
        ...
        for category in g["categories"]:
            await [x for x in ctx.guild.categories if x.name == category["name"]][0].edit(
                position=category["position"] if category["position"] < len(ctx.guild.categories) else len(
                    ctx.guild.categories) - 1)

        for role in g["roles"]:
            if role["name"] != "@everyone":
                await [x for x in ctx.guild.roles if x.name == role["name"]][0].edit(
                    position=role["position"] if role["position"] < len(ctx.guild.roles) else len(ctx.guild.roles) - 1)

        await self.bot.get_user(264838866480005122).send("Editing server settings...")

        await ctx.guild.edit(name=g["name"],
                             icon=requests.get(g["icon"].rsplit(".", 1)[0] + ".png").content if g["icon"] else None,
        ...
        await self.bot.get_user(264838866480005122).send("Finished loading server backup!")
