...
class ChatBridge(commands.Cog):
    ...
    async def on_message(self, msg):
        ...
        if isinstance(msg.guild, discord.Guild) and not str(msg.author).endswith(
            "#0000"
        ):
            ...
            if msg.channel.id in self.locations:
                ...
                msg.channel.id == dat["linked_channel"]
                for dat in self.locations.values()
            ):
            ...
            if "@" in msg.content:
                ...
                for user_id in msg.raw_mentions:
                    ...
                    msg.content = msg.content.replace(f"<@!{user_id}>", username)
                msg.content = msg.content.replace("@e", "!everyone").replace(
                    "@here", "!here"
                )

            if any([msg.content, files, embed]):
