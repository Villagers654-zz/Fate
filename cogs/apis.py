...
class APIS(commands.Cog):
	...
	async def _tenor(self, ctx, *, search):
		...
		if r.status_code == 200:
			...
			anon_id = ""
		r = requests.get(
			"https://api.tenor.com/v1/search?q=%s&key=%s&limit=%s&anon_id=%s" % (search, apikey, lmt, anon_id))
		if r.status_code == 200:
			try:
	...
	async def reddit(self, ctx, search):
		...
		try:
			...
			if subreddit.over18 and not ctx.channel.is_nsfw():
				return await ctx.send(f"Channel '{ctx.channel.name}' needs to be NSFW to view this subreddit")
