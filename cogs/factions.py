...
class Factions(commands.Cog):
    ...
    async def get_members(self, ctx, faction):
        ...
        counter = 0
        ids = []
        limit = self.factions[guild_id][faction]["limit"]
        index = 1
        if (
            self.factions[guild_id][faction]["owner"]
            not in self.factions[guild_id][faction]["members"]
        ):
        ...
        for member_id in self.factions[guild_id][faction]["members"]:
            ...
            if (
                ...
                or member not in ctx.guild.members
                or member.id in ids
            ):
        ...
        for boost, date in boosts.items():
            ...
            if date_string:
                ...
                if time < 1 and boost == "anti-raid":
                    active_boosts["Anti-Raid"] = self.bot.utils.get_time(
                        86400 - seconds
                    )
                time = (datetime.now() - date).seconds / 60 / 60
                if time < 2 and boost == "extra-income":
                    active_boosts["Extra-Income"] = self.bot.utils.get_time(
                        7200 - seconds
                    )
                time = (datetime.now() - date).seconds / 60 / 60
                if time < 2 and boost == "land-guard":
                    active_boosts["Land-Guard"] = self.bot.utils.get_time(
                        7200 - seconds
                    )
            ...
            if faction:
                ...
                if not base:
                    if "channel" in self.factions[guild_id][faction]:
                        base = self.bot.get_channel(
                            self.factions[guild_id][faction]["channel"]
                        )
                if not base:
                    return
                ...
                if str(base.id) not in self.land_claims[guild_id][faction]:
                    self.land_claims[guild_id][faction][str(base.id)] = str(
                        datetime.now()
                    )
                    ...
                    for overwrite in base.overwrites.keys():
                        if isinstance(overwrite, discord.Member):
                            if (
                                ...
                                if (
                                    overwrite.id != self.bot.user.id
                                    and overwrite.id != base.guild.roles[0].id
                                ):
                                    await base.set_permissions(
    ...
    async def _set_icon(self, ctx, url=None):
        ...
        if "icon" not in self.factions[guild_id][faction]:
            await ctx.send(
                f"Buying access to icons will cost you $250\n"
    ...
    async def _set_banner(self, ctx, url=None):
        ...
        if "banner" not in self.factions[guild_id][faction]:
            await ctx.send(
                f"Buying access to banners will cost you $500\n"
                "Reply with .confirm to purchase"
            )
            msg = await self.bot.utils.wait_for_msg(ctx)
            if not msg:
    ...
    async def _join(self, ctx, *, faction):
        ...
        e.description = (
            f"**{ctx.author.name} joined {faction}**\n"
    ...
    async def _pay(self, ctx, *, args):
        ...
        if not amount or len(amount) > 1:
            return await ctx.send(
                "Amount is either missing or I cant find it in your msg"
            )
    ...
    async def _raid(self, ctx, *, faction):
        ...
        if t["boosts"]["anti-raid"] is not None:
            ...
            if (datetime.now() - date).days < 1:
                ...
                e.description = (
                    f"{target} is currently reinforced\n"
        ...
        try:
            ...
            return await ctx.send(
                "Sorry, but one of you is too poor, and I don't speak broke"
            )
    ...
    async def _claim(self, ctx, channel: discord.TextChannel = None):
        ...
        for fac, land_claims in list(self.land_claims[guild_id].items()):
            ...
            if channel_id in land_claims:
                ...
                for boost, time in boosts.items():
                    if boost == "Land-Guard":
                        return await ctx.send(
                            f"This claim is currently guarded, try again in {time} hours"
                        )
                cost += 250
                old_claim = fac
                break
        await ctx.send(
            f"Claiming this channel will cost you ${cost}\n"
            "Reply with .confirm to purchase"
        )
        msg = await self.bot.utils.wait_for_msg(ctx)
        if not msg:
        ...
        if self.factions[guild_id][faction]["balance"] < cost:
            return await ctx.send(
                "You don't have enough money to purchase this channel"
            )
        if old_claim:
    ...
    async def _unclaim(self, ctx, channel: discord.TextChannel = None):
        ...
        if channel_id not in self.land_claims[guild_id][faction]:
            return await ctx.send(
                "You need to claim this channel in order to unclaim it"
            )
        await ctx.send(
            f"Unclaiming this channel will give you $250\n"
    ...
    async def _top(self, ctx):
        ...
        for faction, balance in sorted(factions, key=lambda kv: kv[1], reverse=True)[
            :8
        ]:
            owner = self.bot.get_user(self.factions[guild_id][faction]["owner"])
            if rank == 1:
    ...
    async def _buy(self, ctx, *, item_name):
        ...
        if item == "icon":
            ...
            if "extra-income" in item:
                self.factions[guild_id][faction]["boosts"]["extra-income"] = str(
                    datetime.now()
                )
            if "land-guard" in item:
                self.factions[guild_id][faction]["boosts"]["land-guard"] = str(
                    datetime.now()
                )
                ...
                for faction, land_claims in self.land_claims[guild_id].items():
                    if channel_id in land_claims:
                        ...
                        if self.counter[guild_id][faction] == 5:
                            ...
                            if self.factions[guild_id][faction]["boosts"][
                                "extra-income"
                            ]:
                                date_string = self.factions[guild_id][faction][
                                    "boosts"
                                ]["extra-income"]
