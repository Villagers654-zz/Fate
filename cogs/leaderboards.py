...
class Leaderboards(commands.Cog):
    ...
    def save_data(self):
        with open("./data/userdata/rolerewards.json", "w") as f:
            json.dump(
                {"toggle": self.toggle, "role_rewards": self.role_rewards},
                f,
                ensure_ascii=False,
            )
    ...
    def calc_lvl(self, total_xp):
        ...
        return {"level": round(level), "level_up": round(lvl_up), "xp": round(progress)}
    ...
    async def xp_dump_task(self):
        while True:
            try:
                    ...
                    json.dump(
                        {
                            ...
                            "guilded": self.guilds_data,
                            "monthly_global": self.monthly_global_data,
                            "monthly_guilded": self.monthly_guilds_data,
                            "vclb": self.vclb,
                            "gvclb": self.gvclb,
                        },
                        outfile,
                ...
                try:
                    await self.bot.get_channel(501871950260469790).send(
                        f"Error saving xp: {e}"
                    )
                except:
                    pass
    ...
    async def subtract_spam_from_monthly(self, guild_id, user_id):
        deleted = 0
        for msg_id, msg_time in sorted(
            self.monthly_global_data[user_id].items(),
            key=lambda kv: kv[1],
            reverse=True,
        ):
            if float(msg_time) > time() - 60:
                ...
                deleted += 1
        for msg_id, msg_time in sorted(
            self.monthly_guilds_data[guild_id][user_id].items(),
            key=lambda kv: kv[1],
            reverse=True,
        ):
            if float(msg_time) > time() - 60:
    ...
    async def rank_card(self, ctx, user: discord.Member = None):
        ...
        guild_rank = 0
        for id, xp in sorted(
            self.guilds_data[guild_id].items(), key=lambda kv: kv[1], reverse=True
        ):
            guild_rank += 1
            if user_id == id:
                break
        ...
        ranking = (
            f"Rank: [{guild_rank}] Level: [{level}] XP: [{xp}/{max_xp}]\n\n"
        ...
        color = tuple(
            int(str(user.color).lstrip("#")[i : i + 2], 16) for i in (0, 2, 4)
        )
        ...
        card.putalpha(alpha)
        avatar = Image.open(BytesIO(requests.get(user.avatar_url).content)).convert(
            "RGBA"
        )
    ...
    async def leaderboard(self, ctx):
        ...
        def lb():
            ...
            for user_id, xp in (
                sorted(
                    self.guilds_data[str(ctx.guild.id)].items(),
                    key=lambda kv: kv[1],
                    reverse=True,
                )
            )[:15]:
            ...
            e.set_footer(
                text=self.msg_footer().replace("[result]", f"XP Cleanup: {result}")
            )
            return e
        ...
        def glb():
            ...
            rank = 1
            for user_id, xp in (
                sorted(self.global_data.items(), key=lambda kv: kv[1], reverse=True)
            )[:15]:
                name = "INVALID-USER"
                user = self.bot.get_user(int(user_id))
                if isinstance(user, discord.User):
        ...
        def mlb():
            ...
            e.description = ""
            rank = 1
            for user_id, xp in (sorted(xp.items(), key=lambda kv: kv[1], reverse=True))[
                :15
            ]:
                name = "INVALID-USER"
                user = self.bot.get_user(int(user_id))
        ...
        def gglb():
            ...
            for guild_id, xp in (
                sorted(
                    {i: sum(x.values()) for i, x in self.guilds_data.items()}.items(),
                    key=lambda kv: kv[1],
                    reverse=True,
                )
            )[:8]:
        ...
        def vclb():
            ...
            for user_id, xp in (
                sorted(
                    self.vclb[str(ctx.guild.id)].items(),
                    key=lambda kv: kv[1],
                    reverse=True,
                )
            )[:15]:
        ...
        def gvclb():
            ...
            rank = 1
            for user_id, xp in (
                sorted(self.gvclb.items(), key=lambda kv: kv[1], reverse=True)
            )[:15]:
                name = "INVALID-USER"
                user = self.bot.get_user(int(user_id))
                if isinstance(user, discord.User):
            ...
            if isinstance(embeds[index], list):
                if index == len(embeds) - 1:
                    embeds[index][sub_index].set_footer(
                        text="Last Page! You've reached the end"
                    )
    ...
    async def leaderboards(self, ctx):
        return await ctx.send(
            "This command is currently disabled, xp is still being collected"
        )
        result = await self.run_xp_cleanup()
        e = discord.Embed(color=0x4A0E50)
        ...
        for user_id, xp in (sorted(xp.items(), key=lambda kv: kv[1], reverse=True))[
            ...
            rank += 1
        e.add_field(
            name="Global Monthly Leaderboard", value=gmleaderboard, inline=False
        )
        vcleaderboard = ""
        rank = 1
        for user_id, xp in (
            sorted(
                self.vclb[str(ctx.guild.id)].items(), key=lambda kv: kv[1], reverse=True
            )
        )[:15]:
            name = "INVALID-USER"
            user = self.bot.get_user(int(user_id))
    ...
    async def _card(self, ctx):
        ...
        e.set_image(
            url="attachment://"
            + os.path.basename("/data/images/backgrounds/results/galaxy.png")
        )
        await ctx.send(
            file=discord.File(
                "./data/images/backgrounds/results/galaxy.png",
    ...
    @commands.bot_has_permissions(
        embed_links=True, manage_messages=True, manage_roles=True
    )
    async def rolerewards(self, ctx, *args):
        ...
        if args[0].isdigit():
            ...
            try:
                ...
                if "yes" in msg.content.lower():
                    ...
                    stacking = False
                self.role_rewards[guild_id][args[0]] = {
                    "role": role.id,
        ...
        if args[0] == "remove":
            if guild_id not in self.role_rewards:
                return await ctx.send(
                    "This server currently doesn't have any roles added"
                )
            if not len(args) > 1:
                return await ctx.send(
                    "Role name is a required argument that is missing"
        ...
        if isinstance(m.guild, discord.Guild):
            ...
            if not m.author.bot:
                ...
                if self.cd[user_id] < time():
                    ...
                    previous_level = str(
                        self.calc_lvl(self.guilds_data[guild_id][user_id])["level"]
                    )
                                        ...
                                        if role not in m.author.roles:
                                            try:
                                                ...
                                                return await m.channel.send(
                                                    "Failed to give you your role rewards"
                                                )
                                            e = discord.Embed(color=colors.fate())
                                            ...
                                            try:
                                                ...
                                                await m.channel.send(
                                                    f"Congratulations {m.author.mention}, you already unlocked {role.name}, but here you go anyways"
                                                )
                            if new_level in self.role_rewards[guild_id]:
                                stacking = bool(
                                    self.role_rewards[guild_id][new_level]["stacking"]
                                )
                                if not stacking:
                                    for level, dat in list(
                                        self.role_rewards[guild_id].items()
                                    ):
                                        if not bool(dat["stacking"]):
                                            ...
                                            if role in m.author.roles:
                                                try:
                                                    ...
                                                    return await m.channel.send(
                                                        "Failed to give you your role reward"
                                                    )
                                ...
                                try:
                                    ...
                                    await m.channel.send(
                                        f"Congratulations {m.author.mention}, you unlocked {role.name}"
                                    )
        ...
        if isinstance(user.guild, discord.Guild):
            ...
            async def wrap(channel):
                ...
                for member_id in list(self.cache[cid]["members"].keys()):
                    seconds = (
                        datetime.now() - self.cache[cid]["members"][member_id]
                    ).seconds
            ...
            if before.channel and after.channel:
                if before.channel.id != after.channel.id:
                    ...
                    if user_id in self.cache[channel_id]["members"]:
                        seconds = (
                            datetime.now() - self.cache[channel_id]["members"][user_id]
                        ).seconds
