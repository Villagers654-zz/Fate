...
class Profiles(commands.Cog):
    ...
    async def xprofile(self, ctx, user: discord.Member = None):
        ...
        if user_id not in self.gvclb():
            ...
            vc_xp = str(self.gvclb()[user_id] / 60)[
                : str(self.gvclb()[user_id] / 60).find(".")
            ]
        payload = (
        ...
        draw.text(
            ...
            f"{payload}",
            (0, 0, 255),
            font=ImageFont.truetype("Modern_Sans_Light.otf", 65),
        )
        draw.text(
            (10, 975),
            f"{self.info[str(user.id)]}",
            (255, 255, 255),
            font=ImageFont.truetype("Modern_Sans_Light.otf", 40),
        )
        img = img.convert("RGB")
    ...
    async def profile(self, ctx, user=None):
        ...
        if check is not 0:
            if user.bot == True:
                ...
                e.description = (
                    f"**Level:** {level} **XP:** {str(xp)[:str(xp).find('.')]}\n"
