...
class Psutil(commands.Cog):
    ...
    async def devstats(self, ctx):
        ...
        try:
            cpufreq = bytes2human(psutil.cpu_freq())
        except:
            cpufreq = "unavailable"
        try:
        ...
        e.set_footer(
            text=f"{[round(i) for i in psutil.cpu_percent(interval=1, percpu=True)]}"
        )
    ...
    async def ram(self, ctx):
        await ctx.send(
            f"[{bytes2human(psutil.virtual_memory())}, {p.bytes2human(psutil.virtual_memory().total)}]"
        )
