...
class Core(commands.Cog):
        ...
        *,
        ...
        if not location:
            if command in conf["global"]:
                ...
                channel_id in conf["channels"]
                and command in conf["channels"][channel_id]
            ):
                conf["channels"][channel_id].remove(command)
                await ctx.send(f"Enabled {command} in {ctx.channel.mention}")
            elif (
                ctx.channel.category
                and category_id in conf["categories"]
                and command in conf["categories"][category_id]
            ):
    ...
    async def wiki(self, ctx, *, query: str):
        try:
            ...
            e.description = "Result: {}```{}```For more information, visit [here]({})".format(
    ...
    async def on_message(self, msg: discord.Message):
        if (
            "who is joe" in str(msg.content).lower()
            or "who's joe" in str(msg.content).lower()
        ):
