...
class Ranking(commands.Cog):
    ...
    async def profile(self, ctx):
        ...
        guild_xp = await self.bot.select(
            f"select * from msg where guild_id = {int(guild_id)} order by xp desc;",
            all=True,
        )
        guild_rank = 0
        ...
        if "global" in ctx.message.content or "profile" in ctx.message.content.lower():
            global_xp = await self.bot.select(
                f"select * from global_msg order by xp desc;", all=True
            )
            guild_rank = 0
    ...
    async def leaderboard(self, ctx):
        ...
        results = await self.bot.select("select * from msg order by xp desc;", all=True)
        msg_xp = {}
        ...
        results = await self.bot.select(
            "select * from vc_xp order by xp desc;", all=True
        )
        vc_xp = {}
        for g_id, user_id, xp in results:
        ...
        results = await self.bot.select(
            "select * from global_msg order by xp desc;", all=True
        )
        ...
        results = await self.bot.select(
            "select * from global_vc order by xp desc;", all=True
        )
        ...
        leaderboards["Server Vc Leaderboard"] = {
            guild_id: timedelta(seconds=sum([xp for xp in dat.values()]))
