...
class DMChannel(commands.Cog):
    ...
    @commands.bot_has_permissions(
        embed_links=True, attach_files=True, manage_webhooks=True
    )
    ...
    async def on_message(self, msg: discord.Message):
        if self.channel and self.user:
            if msg.author.id == self.user.id or msg.channel.id == self.channel:
                ...
                if not msg.webhook_id and not msg.content.startswith(
                    "created a dm channel"
                ):
                    ...
                    if isinstance(msg.channel, discord.DMChannel):
                        async with aiohttp.ClientSession() as session:
                            webhook = Webhook.from_url(
                                self.webhook.url, adapter=AsyncWebhookAdapter(session)
                            )
                        ...
                        if msg.channel.id == self.channel:
                            if not self.clean:
                                msg.content = (
                                    f"__**{msg.author.display_name}:**__ {msg.content}"
                                )
                            if msg.attachments:
