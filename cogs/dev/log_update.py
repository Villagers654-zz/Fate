...
class Logger(commands.Cog):
    def __init__(self, bot):
        ...
        self.recent_logs = {}
        for guild_id in self.config.items():
            if self.config[guild_id]["redirects"]:
                self.recent_logs[guild_id] = {
                    event: [] for event in self.config[guild_id]["redirects"].keys()
                }
                self.recent_logs[guild_id]["main"] = []
            else:
                self.recent_logs[guild_id] = []
            ...
            for embed, channelType, logged_at in self.queue[guild_id][-175:]:
                ...
                for i, field in enumerate(embed.fields):
                    if not field.value or field.value is discord.Embed.Empty:
                        self.bot.log(
                            f"A log of type {channelType} had no value", "CRITICAL"
                        )
                    ...
                    if len(field.value) > 1024:
                        ...
                        self.bot.log(
                            f"A log of type {channelType} had had a huge value",
                            "CRITICAL",
                        )
                        for chunk in self.bot.utils.split(str(embed.to_dict()), 1900):
                            self.bot.log(chunk, "CRITICAL")
                ...
                if conf["redirects"] and channelType in conf["redirects"]:
                    channel_id = conf["redirects"][channelType]  # type: int
                else:
                    channel_id = conf["channel"]  # type: int

                # Ensure the channel still exists
                channel = self.bot.get_channel(channel_id)
                if not channel:
                    if channelType not in conf["redirects"]:
                    ...
                    try:
                        ...
                        if channelType in conf["redirects"]:
                            del self.config[guild_id]["redirects"][channelType]
                        else:
                ...
                if guild_id not in self.recent_logs:
                    if self.config[guild_id]["redirects"]:
                        self.recent_logs[guild_id] = {
                            event: []
                            for event in self.config[guild_id]["redirects"].keys()
                        }
                        self.recent_logs[guild_id]["main"] = []
                    else:
                        self.recent_logs[guild_id] = []
                ...
                logs = self.recent_logs[guild_id]
                if isinstance(logs, list):
                    self.recent_logs[guild_id].append([embed, logged_at])
                elif isinstance(logs, dict) and channelType in logs:
                    self.recent_logs[guild_id][channelType].append([embed, logged_at])
                else:
                    self.recent_logs[guild_id]["main"].append([embed, logged_at])

                if isinstance(self.recent_logs[guild_id], dict):
                    # noinspection PyUnboundLocalVariable
                    for Type, logs in self.recent_logs[guild_id].items():
                        for log in logs:
                            if time() - log[1] > 60 * 60 * 24:
                                self.recent_logs[guild_id][Type].remove(log)
                elif isinstance(self.recent_logs[guild_id], list):
    ...
    async def logger(self, ctx):
        if not ctx.invoked_subcommand:
            ...
            e.add_field(
                name="◈ Redirect",
                value="> Change where specific logs get sent to so you can keep logs sorted neatly",
                inline=False,
            )
            ...
            e.add_field(
                ...
                f"\n{p}log redirect - `instructions to redirect`"
    ...
    @logger.command(name="redirect")
    @commands.has_permissions(administrator=True)
    @commands.bot_has_permissions(administrator=True)
    async def _switch(self, ctx, log_type, channel: discord.TextChannel):
        """ Redirects a log type to a different channel """
        pass
            ...
            if guild_id in self.config:
                if self.config[guild_id]["secure"]:
                    if (
                        ...
                        or (msg.channel.id in self.config[guild_id]["redirects"].keys())
                    ):
