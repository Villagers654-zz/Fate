...
class AvapxianRegime(commands.Cog):
    ...
    async def updatestuff(self, ctx, item):
        ...
        if item == "info":
            msg = await self.bot.get_channel(548677537140572160).get_message(
                549484739162275840
            )
            ...
            e.set_footer(
                text=f"Last Updated: {datetime.datetime.now().strftime('%m-%d-%Y %I:%M%p')}"
            )
            await msg.edit(embed=e)
            return await ctx.message.delete()
        if item == "rules":
            msg = await self.bot.get_channel(542568067495100416).get_message(
                542588249646956544
            )
            e = discord.Embed(color=colors.red())
            e.set_thumbnail(
                url="https://cdn.discordapp.com/attachments/507914723858186261/542935248262922270/avapxian_regime_red.png"
            )
            e.description = "yeet"
            fmt = "%m-%d-%Y %I:%M%p"
    ...
    async def _update(self, ctx):
        ...
        msg = await self.bot.get_channel(567821718798860306).fetch_message(
            567823095164043267
        )
        e = discord.Embed(color=colors.cyan())
        ...
        e.set_thumbnail(url=ctx.guild.icon_url)
        e.description = (
            f"■ [4B4T - Gone Sexual](https://discord.gg/BQ23Z2E)\n• Description"
        )
        e.set_footer(
            text=f"Last Updated: {datetime.datetime.now().strftime('%m-%d-%Y %I:%M%p')}"
        )
