...
class Audit(commands.Cog):
    ...
    async def _audit(self, ctx):
        if not ctx.invoked_subcommand:
            ...
            e.add_field(
                name="◈ Commands ◈",
                value=".audit last {amount (default 1)}\n"
            ...
            e.add_field(
                ...
                "[click here](https://discordpy.readthedocs.io/en/rewrite/api.html#discord.AuditLogAction)",
                inline=False,
            )
    ...
    async def _action(self, ctx, action, amount=1):
        ...
        async for entry in ctx.guild.audit_logs(limit=amount, action=action):
            ...
            try:
                e.description += (
                    "\n✦ {0.user.name} | {0.action} to {0.target.name}".format(
                        entry
                    ).replace("AuditLogAction.", "")
                )
            except discord.DiscordException:
                e.description += "\n✦ {0.user} | {0.action} to {0.target}".format(
                    entry
                ).replace("AuditLogAction.", "")
            audit_position += 1
    ...
    async def _user(self, ctx, user: discord.Member, amount=1):
        ...
        async for entry in ctx.guild.audit_logs(limit=amount, user=user):
            if len(e.description) + 75 > 2000:
                reached_char_limit = "| 2k Character Limit Reached"
                break
            e.description += "\n✦ **{0.target}:** {0.action}".format(entry).replace(
                "AuditLogAction.", ""
            )
            audit_position += 1
        e.set_footer(text=f"Total Found: {audit_position} {reached_char_limit}")
    ...
    async def _target(self, ctx, user: discord.Member, amount=1):
        e = discord.Embed(color=colors.cyan())
        e.set_author(
            name=f"Last {amount} action(s) against {user}", icon_url=user.avatar_url
        )
        e.description = ""
        reached_char_limit = ""
        audit_position = 0
        async for entry in ctx.guild.audit_logs():
            ...
            if entry.target is user:
                e.description += "\n✦ **{0.user}:** {0.action}".format(entry).replace(
                    "AuditLogAction.", ""
                )
                audit_position += 1
