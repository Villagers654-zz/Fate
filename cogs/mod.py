...
class Mod(commands.Cog):
    ...
    def convert_timer(self, timer):
        if "d" in timer:
            ...
            if "h" in timer:
                ...
                if "m" in timer:
                    time = timer.replace("m", " minutes").replace(
                        "1 minutes", "1 minute"
                    )
                else:
                    time = timer.replace("s", "seconds").replace(
                        "1 seconds", "1 second"
                    )
        if "d" in str(timer):
            try:
        ...
        if guild_id in self.timers["mute"]:
            for user_id in self.timers["mute"][guild_id].keys():
                ...
                if hours > 0 and minutes > 0:
                    moderation += (
                        f'and {minutes} {"minute" if minutes == 1 else "minutes"} '
                    )
                else:
                    moderation += (
        ...
        if guild_id in self.timers["ban"]:
            for user_id in self.timers["ban"][guild_id].keys():
                ...
                if moderation:
                    moderations.append([seconds, moderation])
        mod_log = ""
        for seconds, moderation in sorted(
            moderations, key=lambda kv: kv[0], reverse=True
        ):
            mod_log += moderation
    ...
    async def purge(self, ctx, *args):
        ...
        if args[0].isdigit():  # no special option used
            ...
            if amount > 1000:
                ...
                return await ctx.send(
                    "You cannot purge more than 1000 messages at a time"
                )
            try:
                await ctx.message.channel.purge(limit=amount, before=ctx.message)
                await ctx.send(
                    f"{ctx.author.mention}, successfully purged {amount} messages",
                    delete_after=5,
                )
        ...
        if ctx.message.mentions:
            ...
            if amount > 250:
                del self.purge[channel_id]
                return await ctx.send(
                    "You cannot purge more than 250 user messages at a time"
                )
            try:
                position = 0
                    ...
                    if msg.author.id == user.id:
                        if msg.id != ctx.message.id:
                            ...
                            if position == amount:
                                break
                await ctx.send(
                    f"{ctx.author.mention}, purged {position} messages from {user.display_name}",
                    delete_after=5,
                )
                return await ctx.message.delete()
            except discord.errors.Forbidden as e:
                ...
                async for msg in ctx.channel.history(limit=500):
                    if msg.attachments:
                        ...
                        if position == amount:
                            break
                await ctx.send(
                    f"{ctx.author.mention}, purged {position} images", delete_after=5
                )
                return await ctx.message.delete()
            except discord.errors.Forbidden as e:
                await ctx.send(e)
                ...
                async for msg in ctx.channel.history(limit=500):
                    if msg.embeds:
                        ...
                        if position == amount:
                            break
                await ctx.send(
                    f"{ctx.author.mention}, purged {position} embeds", delete_after=5
                )
                return await ctx.message.delete()
            except discord.errors.Forbidden as e:
                await ctx.send(e)
                ...
                async for msg in ctx.channel.history(limit=500):
                    if not msg.author.bot:
                        ...
                        if position == amount:
                            break
                await ctx.send(
                    f"{ctx.author.mention}, purged {position} user messages",
                    delete_after=5,
                )
                return await ctx.message.delete()
            except discord.errors.Forbidden as e:
        ...
        if option == "bot" or option == "bots":
            if amount > 250:
                return await ctx.send(
                    "You cannot purge more than 250 bot messages at a time"
                )
            try:
                position = 0
                async for msg in ctx.channel.history(limit=500):
                    if msg.author.bot:
                        ...
                        if position == amount:
                            break
                await ctx.send(
                    f"{ctx.author.mention}, purged {position} bot messages",
                    delete_after=5,
                )
                return await ctx.message.delete()
            except discord.errors.Forbidden as e:
        ...
        if option == "mention" or option == "mentions":
            if amount > 250:
                return await ctx.send(
                    "You cannot purge more than 250 mentions at a time"
                )
            try:
                position = 0
                async for msg in ctx.channel.history(limit=500):
                    if msg.mentions:
                        ...
                        if position == amount:
                            break
                await ctx.send(
                    f"{ctx.author.mention}, purged {position} mentions", delete_after=5
                )
                return await ctx.message.delete()
            except discord.errors.Forbidden as e:
                await ctx.send(e)
        ...
        if option == "reaction" or option == "reactions":
            if amount > 250:
                return await ctx.send(
                    "You cannot purge more than 250 reactions at a time"
                )
            try:
                position = 0
                async for msg in ctx.channel.history(limit=500):
                    if msg.reactions:
                        ...
                        if position == amount:
                            break
                await ctx.send(
                    f"{ctx.author.mention}, purged {position} reactions", delete_after=5
                )
                return await ctx.message.delete()
            except discord.errors.Forbidden as e:
                await ctx.send(e)
                ...
                if phrase.lower() in msg.content.lower():
                    if msg.id != ctx.message.id:
                        ...
                        if position == amount:
                            break
            await ctx.send(
                f"{ctx.author.mention}, purged {position} messages", delete_after=5
            )
            return await ctx.message.delete()
        except discord.errors.Forbidden as e:
            await ctx.send(e)
    ...
    async def kick(self, ctx, user: discord.Member, *, reason="unspecified"):
        ...
        try:
            await user.send(
                f"You have been kicked from **{ctx.guild.name}** by **{ctx.author.name}** for `{reason}`"
            )
        except:
            pass
    ...
    @commands.bot_has_permissions(
        embed_links=True, manage_messages=True, ban_members=True
    )
    async def _softban(
        self, ctx, user: discord.Member, timer="0s", *, reason="unspecified reasons"
    ):
        if user.top_role.position >= ctx.author.top_role.position:
            return await ctx.send("That user is above your paygrade, take a seat")
        if user.top_role.position >= ctx.guild.me.top_role.position:
        ...
        path = (
            os.getcwd()
            + "/data/images/reactions/beaned/"
            + random.choice(os.listdir(os.getcwd() + "/data/images/reactions/beaned/"))
        )
        file = discord.File(path, filename=os.path.basename(path))
        e = discord.Embed(color=colors.fate())
        e.set_image(url="attachment://" + os.path.basename(path))
        await ctx.send(
            f"◈ {ctx.author.display_name} banned {user} ◈", file=file, embed=e
        )
        await ctx.message.delete()
        timer, time = self.convert_timer(timer)
        try:
            await user.send(
                f"You've been banned from **{ctx.guild.name}** by **{ctx.author.name}** for `{reason}` for {time}"
            )
        except:
            pass
        if not isinstance(timer, float):
            return await ctx.send(
                "Invalid character used in timer field\nYou'll have to manually unban this user"
            )
        guild_id = str(ctx.guild.id)
        user_id = str(user.id)
        ...
        timer_info = {
            "user": user.id,
            "time": str(now),
            "end_time": str(now + timedelta(seconds=timer)),
        }
        ...
        if user_id in self.timers["ban"][guild_id]:
            try:
                ...
                await ctx.send(f"Failed to unban {user.name}: {e}")
            else:
    ...
    async def _massnick(self, ctx, *, nick=None):
        ...
        if guild_id in self.massnick:
            if self.massnick[guild_id] is True:
                return await ctx.send(
                    "Please wait until the previous mass-nick is complete"
                )
        if not nick:
            nick = ""
    ...
    async def role(self, ctx, user, role, *, args=""):
        ...
        if role in user.roles:
            ...
            if timers:
                ...
                await ctx.send(
                    f"I'll keep this stuck to them for {', '.join(expanded_timer)}"
                )
                if "-np" not in args:
    ...
    async def massrole(self, ctx, *, role):
        ...
        members = [
            ...
            if role not in m.roles and m.top_role.position < bot.top_role.position
        ]
        msg = await ctx.send(
            f'Estimated time: {str(len(members)) + " seconds" if len(members) < 60 else utils.get_time(len(members))}'
        )
        index = 1
        counter = 0
        total = len(members)
        for member in members:
            ...
            if counter == 10:  # update progress/estimate every 10 members
                estimate = str(
                    str(total) + " seconds" if total < 60 else utils.get_time(total)
                )
                await msg.edit(
                    content=f"Estimated time: {estimate}\n{index}/{len(members)} members updated"
                )
                counter = 0
            index += 1
            total -= 1
    ...
    async def mass_remove_role(self, ctx, *, role):
        ...
        members = [
            m
            for m in ctx.guild.members
            if role in m.roles and m.top_role.position < bot.top_role.position
        ]
        msg = await ctx.send(
            f'Estimated time: {str(len(members)) + " seconds" if len(members) < 60 else utils.get_time(len(members))}'
        )
    ...
    async def mute(self, ctx, user: discord.Member = None, timer=None):
        async with ctx.typing():
            if not user:
                return await ctx.send(
                    "**Format:** `.mute {@user} {timer: 2m, 2h, or 2d}`"
                )
            if user.top_role.position >= ctx.author.top_role.position:
                return await ctx.send("That user is above your paygrade, take a seat")
            guild_id = str(ctx.guild.id)
            ...
            if not mute_role:
                ...
                for channel in ctx.guild.text_channels:
                    try:
                        ...
                        await ctx.send(
                            f"Couldn't modify mute role in {channel.mention}s overwrites"
                        )
                    await asyncio.sleep(0.5)
                for channel in ctx.guild.voice_channels:
                    try:
                        ...
                        await ctx.send(
                            f"Couldn't modify mute role in {channel.name}s overwrites"
                        )
                    await asyncio.sleep(0.5)
            if mute_role in user.roles:
