        ...
        if isinstance(m.guild, discord.Guild):
            if not m.author.bot and m.channel.permissions_for(m.guild.me).send_messages:
                ...
                if str(m.guild.id) not in self.responses:
                    ...
                    with open("./data/userdata/config/toggles.json", "w") as outfile:
                        json.dump(
                            {"responses": self.responses}, outfile, ensure_ascii=False
                        )
                if self.responses[str(m.guild.id)] == "enabled":
                    if list(
                        filter(
                            lambda x: m.content.startswith(x),
                            ["<@506735111543193601>", "<@!506735111543193601>"],
                        )
                    ):
                        m.content = m.content.replace("!", "").replace(
                            "<@506735111543193601> ", ""
                        )
                        found = False
                        ...
                        if found:
                            ...
                            choice = choice.replace(
                                str(self.bot.user.mention), str(m.author.mention)
                            )
