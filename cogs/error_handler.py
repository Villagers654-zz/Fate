...
class ErrorHandler(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        ...
        p = subprocess.Popen(
            "cat  /home/luck/.pm2/logs/fate-error.log",
        ...
        e.set_author(
            name=f"| Fatal Error | {ctx.command}", icon_url=ctx.author.avatar_url
        )
        ...
        if not self.bot.get_user(data.user_id).bot:
            if data.channel_id == 577661392098820106:
                if str(data.emoji) == "✔":
                    ...
                    for embed in msg.embeds:
                        await self.bot.get_channel(577661461543780382).send(
                            "Error Dismissed", embed=embed
                        )
                    await msg.delete()
