...
utility.add_field(name="◈ Main ◈", value="`channelinfo` `servericon` `serverinfo` `userinfo` `addemoji` `delemoji` `makepoll` `welcome` `farewell` `owner` `avatar` `topic` `timer` `limit` `lock` `lockb` `lockm` `note` `wiki` `ud` `id`", inline=False)
