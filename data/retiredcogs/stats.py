        ...
        while True:
            try:
                ...
                if not isinstance(channel, discord.TextChannel):
                    print("4B4T Stats: channel not found")
                    break
                guild = channel.guild
                ...
                leaderboard = ""
                rank = 1
                for user_id, messages in (
                    sorted(
                        list(xp["monthly_guilded"][guild_id].items()),
                ...
                e.add_field(
                    name="◈ Discord ◈",
                    value=f"__**Owner**__: {channel.guild.owner}\n__**Members**__: {channel.guild.member_count}",
                    inline=False,
                )
                e.add_field(name="Leaderboard", value=leaderboard, inline=False)
                e.add_field(
                    name="◈ Memory ◈",
                    value=f"__**Storage**__: [{bytes2human(psutil.disk_usage('/').used)}/{bytes2human(psutil.disk_usage('/').total)}]\n"
