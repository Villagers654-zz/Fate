...
class SelfRoles(commands.Cog):
    ...
    def save_data(self):
        with open("./data/userdata/selfroles.json", "w") as outfile:
            json.dump(
                {"message": self.message, "roles": self.roles},
                outfile,
                ensure_ascii=False,
            )
    ...
    @commands.bot_has_permissions(
        manage_roles=True, manage_messages=True, embed_links=True
    )
                ...
                if msg.content.lower() == "confirm":
                    ...
                    if msg.content.lower() == "cancel":
                        ...
                        if role_one is None:
                            ...
                            e = discord.Embed(color=0x80B0FF)
                            e.set_author(
                                name=ctx.author.name, icon_url=ctx.guild.icon_url
                            )
                                    ...
                                    try:
                                        ...
                                        for channel in ctx.guild.channels:
                                            if (
                                                ...
                                                self.message[str(ctx.guild.id)] = str(
                                                    message.id
                                                )
                                                await embed.delete()
                                        ...
                                        if check == 0:
                                            await ctx.send(
                                                "Channel not found, please restart",
                                                delete_after=10,
                                            )
                                            await embed.delete()
                                else:
